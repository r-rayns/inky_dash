...
def get_image_feed(image_feed_service: ImageFeedService = Provide[Container.image_feed_service]):
    try:
        ...
        if current_image_feed_configuration:
            return jsonify(
                data=current_image_feed_configuration.model_dump()), 200
        else:
            return jsonify(
                message="No existing image feed configuration found",
                data=None), 200
    except Exception as err:
        logger.exception(err)
        logger.error(f"Failed to get existing image feed configuration: {err}")
        return error_response(
            "Error retrieving existing image feed configuration", [str(err)])
...
def set_image_feed(image_feed_service: ImageFeedService = Provide[Container.image_feed_service]):
    try:
        ...
        return jsonify(message="Image feed should now be updating",
                       data=dict(success=True)), 200
    except Exception as err:
        logger.exception(err)
