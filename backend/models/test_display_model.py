...
import pytest
from display_model import DisplaySettings, ColourPalette, BorderColour
import base64
...
def test_valid_display_settings():
    ...
    try:
        validated_settings = DisplaySettings(colour_palette=ColourPalette.red,
...
def test_non_png_image():
    ...
    with pytest.raises(ValidationError):
        DisplaySettings(colour_palette=ColourPalette.red,
                        border_colour=BorderColour.black,
                        change_delay=200,
                        images=[base64_jpg])
...
def test_invalid_palette():
    with pytest.raises(ValidationError):
        DisplaySettings(colour_palette="blue",  # type: ignore
                        border_colour=BorderColour.black,
                        change_delay=200,
                        images=[valid_base64_png])
...
def test_png_too_large():
    ...
    png_header = b'\x89PNG\r\n\x1a\n'
    # large byte array representing an image over 100KB
    large_data = png_header + bytearray(150 * 1024)  # 150KB+
    # encode to base64
