...
router.get('',
  asyncErrorHandler(async (req, res, next) => {
    const image_file = await fs
      .readFile(path.join(process.cwd(), 'current_image/inky.png'));
...
router.post('/upload',
  ...
  asyncErrorHandler(async (req, res, next) => {
    ...
    await image_read
      .write(path.join(process.cwd(), 'current_image/inky.png'));
    await pythonScript(
      path.join(process.cwd(), 'python-scripts/display-current-image.py'),
