...
import type {
  ImageFeedConfigurationResponse,
  SetImageFeedResponse,
} from '@/types/image-feed'
...
function ImageFeedConfigurationPage() {
  ...
  const { data: imageFeedData, isLoading: imageFeedDataLoading } =
    useSWR<ImageFeedConfigurationResponse>(constructUrl('image-feed'), {
      revalidateOnReconnect: true,
    })

  const imageFeedForm = useForm<
    ImageFeedFormInput,
    unknown,
    ImageFeedFormOutput
  >({
  ...
  const { submit, responsePending } = useSubmit<
    SetImageFeedResponse | null | void,
    [ImageFeedFormOutput]
  >((values: ImageFeedFormOutput) =>
    updateImageFeed({ ...values }, (message: string) =>
      addToast(`Error updating image feed configuration - ${message}`, {
  ...
  const onSubmit = async (values: ImageFeedFormOutput) => {
    ...
    if (res) {
      addToast('Successfully submitted image feed configuration', {
  ...
  const stepPollingInterval = (increment: boolean = true) => {
    let newPollingInterval: second = imageFeedForm.getValues(
      'pollingInterval',
    ) as second
    if (increment) {
      newPollingInterval = Math.min(
        maxPollInterval,
        newPollingInterval + 60,
      ) as second
    } else {
      newPollingInterval = Math.max(
        minPollInterval,
        newPollingInterval - 60,
      ) as second
    }
    ...
    <Form {...imageFeedForm}>
      ...
      >
        <div className="max-w-prose w-full flex flex-col gap-y-4">
          {imageFeedDataLoading && <Skeleton className="h-8" />}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input {...field} placeholder="Image Feed URL" />
                  </FormControl>
                  <FormDescription>
                    The image URL which will be periodically polled.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormDescription>
                    Period of time between each poll. The display will only be
                    updated if the image has changed.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
...
const imageFeedConfigurationFormSchema = z.object({
  pollingInterval: z
    .number()
    .transform((value: number): second => value as second),
  imageFeedUrl: z
    .string()
    .startsWith('http://', { message: protocolErrorMessage })
    .or(z.string().startsWith('https://', { message: protocolErrorMessage }))
    .and(
      z
        .string()
        .endsWith('.png', { message: urlFileTypeErrorMessage })
