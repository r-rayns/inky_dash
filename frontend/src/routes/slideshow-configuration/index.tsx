...
function SlideshowConfigurationPage() {
  ...
  const { slideshowDataLoading, slideshowForm, submitSlideshowConfiguration } =
    useSlideshow()

  const { displaySettings } = useSettings()
  const displayClass =
    displaySettings?.type && displayClassByType[displaySettings.type]
  const { submit, responsePending } = useSubmit<boolean, [SlideshowFormOutput]>(
    (values: SlideshowFormOutput) => submitSlideshowConfiguration(values),
  )
  ...
  const stepChangeDelay = (increment: boolean = true) => {
    let newChangeDelay: second = (slideshowForm?.getValues('changeDelay') ??
      minChangeDelay) as second
    if (increment) {
      ...
      <Form {...slideshowForm}>
        ...
        >
          <div className="max-w-prose w-full">
            {slideshowDataLoading && <Skeleton className="h-8" />}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>
                      Period of time an image is displayed for.
                    </FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
