  ...
  return (
    <>
      <div
        ...
        ref={mockDisplayRef}
      >
        <div className={`bg-white flex overflow-auto max-w-full max-h-[500px]`}>
          {settingsIsLoading ? (
        ...
        {images.length > 0 && (
          <p className="absolute top-0 left-0 px-1 font-mono text-xs m-0 backdrop-blur-sm bg-black/50 text-slate-300">
            Image {imageIndex + 1} of {images.length} - Dimensions:{' '}
        ...
        <div className="flex justify-evenly flex-row mt-1 mx-auto ">
          ...
          <Button
            ...
            onClick={nextImage}
            disabled={
              imageIndex === Math.max(0, images.length - 1) || settingsIsLoading
            }
            variant="icon"
            title="Next image"
