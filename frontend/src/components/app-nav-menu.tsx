        ...
        <NavigationMenuItem>
          ...
          <NavigationMenuContent>
            <div className="flex flex-row">
              <div className="hidden w-44 m-1 p-2 bg-gradient-to-b from-background/20 to-background/80 rounded-md md:flex items-center text-center">
                <p className="text-sm">{activeDescription}</p>
              </div>
              <ul
                className="m-2 p-2 list-none"
                onMouseLeave={() => setActiveDescription(defaultDescription)}
              >
                {configurationLinks.map((link) => {
                  return (
                    <li key={link.name} className="mt-1 first:mt-0 text-sm">
                      <NavigationMenuLink
