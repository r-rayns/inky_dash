...
export default function SettingsCard({
  ...
  const [detectedDisplay, setDetectedDisplay] = useState<InkyDisplay | null>(
    null,
  )
  ...
  useEffect(() => {
    if (
      detectedDisplayType &&
      detectedDisplayType !== DetectionError.UNSUPPORTED
    ) {
  ...
  return (
    <Card
      className={clsx('max-w-prose w-full ', props.className)}
    ...
    >
        ...
        <div className="grid grid-cols-2">
          ...
          <div>
            ...
            {!isLoading && displaySettings && (
              <p className="text-card-foreground">
                {paletteLabel[displaySettings.colourPalette]}
              </p>
            )}
          </div>
          <div>
            ...
            {!isLoading && displaySettings && (
              <p className="text-card-foreground">
                {capitalize(displaySettings.borderColour)}
              </p>
            )}
          </div>
          <div>
            ...
            {isLoading && <Skeleton className="h-4 w-1/2" />}
            {!isLoading && displaySettings && (
              <Link title="Configure Mode" to={getConfigureModeLink()}>
      ...
      <CardFooter className="flex flex-col items-start gap-2 text-sm w-full">
        <div
          className={clsx(
            'w-full h-auto flex items-center p-2 rounded-md text-sm',
            {
            ...
            if (detectedDisplayType === DetectionError.UNSUPPORTED) {
              return (
                <span className="text-red-700/70 flex items-center">
                  ...
                  <span>
                    Unsupported display detected. See{' '}
            ...
            if (detectedDisplayType === null) {
              return (
                <span className="text-red-700/70 flex items-center">
                  <BadgeAlert size={22} className="inline mr-2" />
                  <span>
                    Automatic display detection failed. See{' '}
                    <Link to="/help">help</Link> section.
                  </span>
                </span>
              )
        ...
        {isLoading ? (
          ...
          <Link className="self-end" to="/display-settings">
            Edit settings...
          </Link>
        )}
      </CardFooter>
