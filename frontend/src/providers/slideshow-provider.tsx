...
export const SlideshowProvider = ({
  ...
  const [slideshowConfig, setSlideshowConfig] =
    useState<SlideshowConfiguration>(defaultSlideshowConfiguration)
  ...
  const { data: slideshowData, isLoading: slideshowDataLoading } =
    useSWR<SlideshowConfigurationResponse>(constructUrl('slideshow'), {
  ...
  async function submitSlideshowConfiguration(
    submittedConfig?: SlideshowConfiguration,
    successMessage?: string,
  ): Promise<boolean> {
    ...
    if (res) {
      const message =
        successMessage ?? 'Successfully submitted slideshow configuration'
      addToast(message, {
