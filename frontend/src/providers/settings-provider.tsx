...
export const SettingsProvider = ({
  children,
  addToast,
}: Readonly<{ children: ReactNode; addToast: AddToastFn }>) => {
  const [detectedDisplay, setDetectedDisplay] = useState<
    DisplayType | DetectionError | null | undefined
  >(undefined)
  ...
  const { data: settingsResponse, isLoading: settingsIsLoading } =
    useSWR<DisplaySettingsResponse>(constructUrl('settings'), {
      revalidateIfStale: true,
      revalidateOnFocus: false,
      revalidateOnReconnect: true,
    })
  ...
  async function submitDisplaySetting(
  ...
  return (
    <SettingsContext.Provider
      value={{
        displaySettings,
        detectedDisplayType: detectedDisplay,
        submitDisplaySetting,
        settingsIsLoading,
      }}
    >
      {children}
